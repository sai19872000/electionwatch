...
function RoundProgress({ completed, total }: { completed: number; total: number }) {
  if (total === 0) return <p className="text-zinc-500 text-sm">Round info unavailable</p>;
    ...
    <div>
      ...
      <div className="h-2.5 bg-zinc-800 rounded-full overflow-hidden">
        <div
          ...
          style={{ width: `${pct}%`, backgroundColor: 'var(--ew-live)' }}
        />
      </div>
    </div>
  );
      ...
      <table className="w-full text-sm" aria-label="Candidates list">
        <thead>
          <tr className="border-b border-zinc-800">
            <th className="text-left py-2 pr-4 text-zinc-500 text-xs font-medium">Candidate</th>
            <th className="text-left py-2 pr-4 text-zinc-500 text-xs font-medium">
              <button onClick={() => setSortKey('party')} className="hover:text-white transition-colors" aria-label="Sort by party">
                Party{sortKey === 'party' && ' ↓'}
              </button>
            </th>
            <th className="text-right py-2 text-zinc-500 text-xs font-medium">
              <button onClick={() => setSortKey('votes')} className="hover:text-white transition-colors" aria-label="Sort by votes">
                Votes{sortKey === 'votes' && ' ↓'}
              </button>
            </th>
          </tr>
        ...
        <tbody>
          {sorted.map((cand, i) => (
            <tr key={i} className="border-b border-zinc-800/50 hover:bg-zinc-800/30 transition-colors">
              <td className="py-2 pr-4 text-white">{cand.candidate}</td>
              ...
              <td className="py-2 text-right text-zinc-300 tabular-nums">
                {cand.votes !== null ? cand.votes.toLocaleString() : <span className="text-zinc-600">TBD</span>}
                {cand.pct !== null && (
                  <span className="text-zinc-600 text-xs ml-1">{cand.pct.toFixed(1)}%</span>
                )}
              </td>
            </tr>
          ))}
...
export function AcDetailClient({ id }: AcDetailClientProps) {
  ...
  const color = c ? partyColor(c.leading_party) : '#6B7280';
  const isDecided = c?.status === 'declared';
  const candidates = c ? buildCandidatesFromConstituency(c) : [];

  const statusView = (() => {
    switch (c?.status) {
      case 'declared':   return { label: 'Result Declared', pill: 'bg-emerald-900/50 text-emerald-400', candidateLabel: 'Winner' };
      case 'counting':   return { label: 'Counting in Progress', pill: 'bg-amber-900/50 text-amber-400', candidateLabel: 'Currently Leading' };
      case 'pending':    return { label: 'Counting Not Started', pill: 'bg-zinc-800 text-zinc-400', candidateLabel: 'Awaiting Count' };
      default:           return { label: 'Counting in Progress', pill: 'bg-zinc-800 text-zinc-300', candidateLabel: 'Currently Leading' };
    }
  })();
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="sticky top-0 z-20 bg-[#0f0f0f]/90 backdrop-blur border-b border-zinc-800 px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-white font-bold text-base truncate">{c?.name ?? `AC ${acNo}`}</h1>
          <p className="text-zinc-500 text-xs">{stateName} · AC #{acNo}</p>
        </div>
      ...
      <main className="flex-1 max-w-lg mx-auto w-full px-4 py-5 space-y-5">
        {isLoading && !stateData && (
          <div className="space-y-3 animate-pulse">
            {[1, 2, 3].map((i) => <div key={i} className="h-20 rounded-xl bg-zinc-800" />)}
          </div>
        )}
        ...
        {c && (
          <>
            ...
            >
              <p className="text-zinc-400 text-xs mb-1">{statusView.candidateLabel}</p>
              <div className="flex items-start justify-between gap-3">
                <h2 className="text-white text-2xl font-extrabold leading-tight">{c.leading_candidate}</h2>
                <span
            ...
            {c.rounds_total > 0 && (
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
                <h3 className="text-zinc-400 text-xs uppercase tracking-wider mb-3">Counting Progress</h3>
            ...
            {/* Candidates */}
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
              <h3 className="text-zinc-400 text-xs uppercase tracking-wider mb-3">Candidates</h3>
              {candidates.length > 0 ? (
                <CandidatesTable candidates={candidates} />
              ) : (
                <p className="text-zinc-600 text-sm">Full candidates list loading…</p>
              )}
              <p className="text-zinc-700 text-xs mt-3">Full candidates data from T2 backend (candidates_{'<state>'}.json)</p>
            </div>

            {/* History block */}
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-3">
              <h3 className="text-zinc-400 text-xs uppercase tracking-wider">Historical Context</h3>
              <div className="text-sm space-y-1">
                <div className="flex justify-between text-zinc-500">
                  <span>2021 Assembly (this AC)</span>
                  <span className="text-zinc-600">T1 data pending</span>
                </div>
                <div className="flex justify-between text-zinc-500">
              ...
              <p className="text-zinc-700 text-xs">Historical baked data from T1 data agent. LS cycles show parent seat rollup.</p>
            </div>
        ...
        {!isLoading && !error && !c && stateData && (
          <p className="text-zinc-500 text-sm">Constituency data not yet available.</p>
        )}
      </main>

      {/* Share row */}
      {c && (
        <div className="border-t border-zinc-800 px-4 py-3 flex items-center gap-3 bg-[#0f0f0f]">
          <span className="text-zinc-600 text-xs flex-1">/ac/{id}</span>
          <button
            onClick={handleShare}
      ...
      <footer className="bg-[#0f0f0f]/95 border-t border-zinc-800">
        <SourcesStrip />
