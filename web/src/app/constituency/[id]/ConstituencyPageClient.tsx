...
function RoundProgress({ completed, total }: { completed: number; total: number }) {
  if (total === 0) {
    return <p className="text-zinc-500 text-sm">Round info unavailable</p>;
  }
...
export function ConstituencyPageClient({ id }: ConstituencyPageClientProps) {
  ...
  const partyColor = c ? getPartyColor(c.leading_party) : '#6B7280';
    ...
    switch (c?.status) {
      case 'declared':
        return {
          ...
          pillClass: 'bg-emerald-900/50 text-emerald-400',
        };
      case 'counting':
        return {
          candidateLabel: 'Currently Leading',
          statusLabel: 'Counting in Progress',
          pillClass: 'bg-amber-900/50 text-amber-400',
        };
      case 'pending':
        return {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <header className="sticky top-0 z-20 bg-[#0f0f0f]/90 backdrop-blur border-b border-zinc-800 px-4 py-3 flex items-center gap-3">
        <Link href={`/state/${stateCode}`} className="text-zinc-400 hover:text-white transition-colors text-sm">
          ← Back
        </Link>
      ...
      <main className="flex-1 max-w-lg mx-auto w-full px-4 py-6 space-y-6">
        ...
        {c && (
          <>
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <h2 className="text-white text-xl font-bold">{c.leading_candidate}</h2>
                </div>
                <span
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
              <h3 className="text-zinc-400 text-xs uppercase tracking-wider mb-2">Status</h3>
              <span
